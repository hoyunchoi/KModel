...
struct KNode : public Node_Epidemic {
    ...
    unsigned m_infectiousNeighbor{0};
...
struct KM {
   ...
   public:
    ...
    std::vector<std::vector<unsigned>> sync_run(const double&, const unsigned&);
    const std::string directoryName(const std::string&, const unsigned&, const unsigned&) const ;
    const std::string fileName(const int&) const ;
};
...
const unsigned KM::m_getQuarantineNeighbor(const unsigned& t_index) const {
    ...
    for (const unsigned& neighbor : m_nodes[t_index].m_neighbors) {
        if (m_nodes[neighbor].m_state == "QA" || m_nodes[neighbor].m_state == "QI") {
            ++quarantineNeighbor;
        }
    }
...
const unsigned KM::m_getInfectiousNeighbor(const unsigned& t_index) const {
    ...
    for (const unsigned& neighbor : m_nodes[t_index].m_neighbors) {
        if (m_nodes[neighbor].m_state == "A" || m_nodes[neighbor].m_state == "I") {
            ++infectiousNeighbor;
        }
    }
...
void KM::m_updateTransitionRate(const unsigned& t_index) {
    ...
    m_nodes[t_index].m_quarantineNeighbor = quarantineNeighbor;
    const int intState = stateToInt.at(m_nodes[t_index].m_state);
    switch (intState) {
        //* S process
        case 0: {
            const unsigned infectiousNeighbor = m_getInfectiousNeighbor(t_index);
            m_nodes[t_index].m_infectiousNeighbor = infectiousNeighbor;
            m_nodes[t_index].m_transitionRate = m_SE * infectiousNeighbor + m_XQX * quarantineNeighbor;
            break;
        }
        //* E process
        case 1: {
            m_nodes[t_index].m_transitionRate = m_EA + m_EI + m_XQX * quarantineNeighbor;
            break;
        }
        //* A process
        case 2: {
            m_nodes[t_index].m_transitionRate = m_AR + m_XQX * quarantineNeighbor;
            break;
        }
        //* I process
        case 3: {
            m_nodes[t_index].m_transitionRate = m_IQI + m_XQX * quarantineNeighbor;
            break;
        }
        //* R process
        case 4: {
            m_nodes[t_index].m_transitionRate = m_XQX * quarantineNeighbor;
            break;
        }
        //* QE process
        case 6: {
            m_nodes[t_index].m_transitionRate = m_EA + m_EI;
            break;
        }
        //* QA process
        case 7: {
            m_nodes[t_index].m_transitionRate = m_AR;
            break;
        }
        //* QI process
        case 8: {
            m_nodes[t_index].m_transitionRate = m_QICR;
            break;
        }
        //* QS, QR, CR process
        default: {
            m_nodes[t_index].m_transitionRate = 0.0;
            break;
        }
    }
}
...
void KM::m_release(const double& t_deltaT) {
    ...
    for (const unsigned& index : quarantineIndex) {
        m_nodes[index].m_quarantineTime += t_deltaT;
        const int intState = stateToInt.at(m_nodes[index].m_state);
        switch (intState) {
            //* QS process
            case 5: {
                if (m_nodes[index].m_quarantineTime > m_tau) {
                    m_nodes[index].m_state = "S";
                    --m_numberOfStates[5];
                    ++m_numberOfStates[0];
                    m_nodes[index].m_quarantineTime = 0.0;
                    m_quarantineIndex.erase(index);
                    m_updateTransitionRate(index);
                    if (m_nodes[index].m_transitionRate) {
...
void KM::m_syncUpdate(const double& t_deltaT) {
    ...
    for (const unsigned& index : m_reactingIndex) {
        const int intState = stateToInt.at(m_nodes[index].m_state);
        const double transitionRate = m_nodes[index].m_transitionRate;
        ...
        switch (intState) {
            ...
            case 0: {
                if (prob <= transitionProb) {
                    ...
                    //* S -> QS
                    if (p * transitionRate < m_nodes[index].m_quarantineNeighbor * m_XQX) {
                        m_nodes[index].m_state = "QS";
                        --m_numberOfStates[0];
                        ++m_numberOfStates[5];
                        m_nodes[index].m_quarantineTime = 0.0;
            ...
            case 1: {
                if (prob <= transitionProb) {
                    ...
                    if (p * transitionRate < m_EA) {
                        m_nodes[index].m_state = "A";
                        --m_numberOfStates[1];
                        ++m_numberOfStates[2];
                    }
                    //* E -> I
                    else if (p * transitionRate < m_EA + m_EI) {
                        m_nodes[index].m_state = "I";
                        --m_numberOfStates[1];
                        ++m_numberOfStates[3];
                    }
                    //* E -> QE
                    else {
                        m_nodes[index].m_state = "QE";
                        --m_numberOfStates[1];
                        ++m_numberOfStates[6];
                        m_nodes[index].m_quarantineTime = 0.0;
                        m_quarantineIndex.emplace(index);
    ...
    for (const unsigned& index : newReactingIndex) {
        ...
        if (m_nodes[index].m_state == "A" || m_nodes[index].m_state == "I") {
            for (const unsigned& neighbor : m_nodes[index].m_neighbors) {
                if (m_nodes[neighbor].m_state == "S") {
                    m_reactingIndex.emplace(neighbor);
                }
            }
        }
        //* Add neighbor S,R of QA,QI node into reacting nodes (quarantine)
        else if (m_nodes[index].m_state == "QA" || m_nodes[index].m_state == "QI") {
            for (const unsigned& neighbor : m_nodes[index].m_neighbors) {
                if (m_nodes[neighbor].m_state == "S" || m_nodes[neighbor].m_state == "R") {
                    m_reactingIndex.emplace(neighbor);
                }
            }
        }
    ...
    while (m_currentTime >= m_nextDate) {
        ++m_nextDate;
        m_data.emplace_back(m_numberOfStates);
    }
    m_updateTransitionRates();
...
std::vector<std::vector<unsigned>> KM::sync_run(const double& t_deltaT, const unsigned& t_maxDate){
...
const std::string KM::directoryName(const std::string& t_networkType, const unsigned& t_networkSize, const unsigned& t_meanDegree) const {
    const std::string directoryName = t_networkType + ",N" + to_stringWithExponent((double)t_networkSize, 1) + ",M" + std::to_string(t_meanDegree) + "/";
    return directoryName;
}

const std::string KM::fileName(const int& t_randomEngineSeed) const {
    const double EAI = m_EA + m_EI;
    const double pA = m_EA/EAI;

    const std::string fileName = "SE" + to_stringWithPrecision(m_SE, 2) + ",EAI" + to_stringWithPrecision(EAI, 2) + ",pA" + to_stringWithPrecision(pA, 2) + ",IQI" + to_stringWithPrecision(m_IQI, 2) + ",AR" + to_stringWithPrecision(m_AR, 2) + ",QICR" + to_stringWithPrecision(m_QICR, 2) + ",XQX" + to_stringWithPrecision(m_XQX, 2) + ",T" + to_stringWithPrecision(m_tau, 2);
